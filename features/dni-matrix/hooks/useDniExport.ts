...
import { exportDniDataToExcel } from '@/lib/dni-matrix/excel-exporter';
...
export function useDniExport({ slotsData, printSize, showToast, playSound }: UseDniExportProps) {
  ...
  // Exportar Data a Excel (.xlsx) con 2 columnas: Nombres y Apellidos, DNI
  const handleExportExcel = () => {
    try {
      const allSlots = Object.values(slotsData);
      exportDniDataToExcel(allSlots, (msg) => {
        playSound('complete');
