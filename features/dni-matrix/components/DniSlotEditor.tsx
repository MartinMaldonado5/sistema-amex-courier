...
export function DniSlotEditor({
  ...
  return (
    <>
      ...
      {/* Campo DNI del expediente */}
      <div className="matrix-slot-metadata">
        <label className="metadata-label">DNI:</label>
        <input
          type="text"
          className="metadata-input"
          placeholder="Ej: 12345678"
          value={activeSlot.dni || ''}
          onChange={(e) => updateSlot({ ...activeSlot, dni: e.target.value.replace(/[^0-9A-Za-z]/g, '').slice(0, 15) })}
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              e.preventDefault();
              (e.target as HTMLInputElement).blur();
      ...
      <button
        ...
        title={
          activeSlot.anverso
            ? 'Extraer automáticamente nombres, apellidos y DNI del Anverso con AMEXito'
      ...
      >
        {isExtractingName ? (
          <>
            ...
            <span className="ai-btn-text">AMEXito está leyendo nombres y DNI...</span>
          </>
        ) : (
          <>
            <div className="ai-btn-left">
              <span className="ai-robot-icon">🤖</span>
              <span className="ai-btn-text">Extraer Nombres y DNI con AMEXito</span>
            </div>
